...
class TestUSBERTStsCalculator(ut.TestCase):
    """
    Test suit for 'SBERTStsCalculator' component
    """
    def setUp(self) -> None:
        """
        Test setup fixture
    ...
    def test_SBERTStsCalculator_Construction_Is_Valid(self):
        """
        Test the validity of 'SBERTStsCalculator' construction
        :return: None
        """
        error_msg = f"Invalid tests: Error testing function: {inspect.stack()[0][3]}()"
        embedder = ss.SBERTStsCalculator(corpus=self.__docs)
        self.assertIsNotNone(embedder, msg=error_msg)

    def test_SBERTStsCalculator_computeSelfSimilarities_Is_Valid(self):
        """
        Test the validity of 'SBERTStsCalculator' similarity measure extracting query from the corpus (itself)
        :return: None
        """
        error_msg = f"Invalid tests: Error testing function: {inspect.stack()[0][3]}()"
        embedder = ss.SBERTStsCalculator(self.__docs)
    ...
    def test_SBERTStsCalculator_computeSimilarities_Is_Valid(self):
        """
        Test the validity of 'SBERTStsCalculator' similarity measure - b/w query and corpus docs
        :return: None
        """
        error_msg = f"Invalid tests: Error testing function: {inspect.stack()[0][3]}()"
        corpus_docs = self.__docs[:40]
        query_docs = self.__docs[40:]
        embedder = ss.SBERTStsCalculator(corpus_docs, query_docs=query_docs)
        results_df = embedder.computeSimilarities()
        self.assertIsNotNone(results_df, msg=error_msg)
        print(u.Helpers.tableize(results_df.head(50)))

    def test_SBERTStsCalculator_createEmbeddingForDocs_Is_Valid(self):
        """
        Test the validity of 'SBERTStsCalculator' compute embeddings for specified docs
        :return: None
        """
        error_msg = f"Invalid tests: Error testing function: {inspect.stack()[0][3]}()"
        corpus_docs = self.__docs[:40]
        corpus_embedding = ss.SBERTStsCalculator.createEmbeddingForDocs(docs=corpus_docs)
